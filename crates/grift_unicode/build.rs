...
fn main() {
    ...
    for line in contents.lines() {
        ...
        let comment = parts[3].trim().trim_start_matches('#').trim().to_string();
    ...
    writeln!(
        out,
        "// Auto-generated from CaseFolding.txt — do not edit manually."
    )
    .unwrap();
    writeln!(
        out,
        "// Source: Unicode CaseFolding.txt (status 'F' entries)"
    )
    .unwrap();
    ...
    writeln!(
        out,
        "/// Auto-generated by build.rs from the official Unicode CaseFolding.txt file."
    )
    .unwrap();
    writeln!(
        out,
    ...
    for (cp, (mapping, comment)) in &entries {
        ...
        assert!(
            len >= 2 && len <= 3,
            "Unexpected mapping length {} for U+{:04X}",
            len,
            cp
        );
